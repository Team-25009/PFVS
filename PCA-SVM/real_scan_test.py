...
def load_training_subset(color_abbrev, data_dir, materials=["PLA", "ASA", "PET"], n_scans=50):
    """
    For a given color (using the abbreviation, e.g., "R" for Red),
    load the first n_scans from each database file corresponding to each material.
...
def plot_decision_boundary(model, X_pca, title, zoom_factor=2.0, 
    ...
    """
    Plot the decision boundary (in PCA space) with an expanded view.
    Optionally overlay two sets of points:
      - X_test_pca: e.g., real scans (plotted as circles)
      - X_train_pca: e.g., training subset scans (plotted as squares)
    """
    # Determine original boundaries from all provided points (if available) or just X_pca.
    ...
    x_half_range = (x_max_orig - x_min_orig) / 2
    y_half_range = (y_max_orig - y_min_orig) / 2

    # Expand boundaries using zoom_factor
    x_half_range_zoom = x_half_range * zoom_factor
    y_half_range_zoom = y_half_range * zoom_factor
    ...
    # Overlay real scan points (if provided)
    if X_test_pca is not None:
        sc1 = plt.scatter(X_test_pca[:, 0], X_test_pca[:, 1],
                          c='black', edgecolor='w', marker='o', s=100,
                          label="Real Scans")
    # Overlay training subset points (if provided)
    if X_train_pca is not None:
        sc2 = plt.scatter(X_train_pca[:, 0], X_train_pca[:, 1],
...
def main():
    # Build file paths based on script location
    script_dir = os.path.dirname(__file__)
    test_file = os.path.join(script_dir, "data", "real_scans.txt")
    data_dir = os.path.join(script_dir, "data")
    models_dir = os.path.join(script_dir, "models_per_color")

    # Load real scans file (assumed to be CSV with header)
    df_real = pd.read_csv(test_file)
    print("Real scans file read successfully!")
    print(df_real.head())

    # Define spectral columns (assuming 18 values)
    spectral_columns = [f"Value{i}" for i in range(1, 19)]
    
    # Mapping full color names (from test file) to model abbreviations
    color_map = {
        "Red": "R",
        "Green": "G",
        "Blue": "B",
        "Black": "K",  # assuming 'K' was used for Black in your training filenames
        "White": "W"
    }
    
    all_predictions = []

    # Process each unique color present in the real scans file
    for full_color in df_real["Color"].unique():
        ...
        # Transform real scans
        X_real_scaled = scaler.transform(X_real)
        X_real_pca = pca.transform(X_real_scaled)
        
        # Predict for real scans (for logging if needed)
        ...
        # Now load training subset: first 50 scans for each filament-material of this color
        ...
        # Plot decision boundary with both real scans and training subset points overlaid
    ...
    # Optionally, combine and save predictions for real scans
    if all_predictions:
